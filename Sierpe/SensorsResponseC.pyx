...
def simulate_pmt_response(int event_number,pmtrd_):
    """
    Sensor Response
...
cpdef np.ndarray rebin_array(np.ndarray[int,ndim=1] a, int stride):
    """
    rebins array a according to stride
    """
    
    cdef int lenb = (len(a)+1)/int(stride)
    #cdef np.ndarray b = np.zeros(lenb, dtype=np.int)
...
cpdef np.ndarray rebin_signal(int event_number,pmtrd_):
    """
